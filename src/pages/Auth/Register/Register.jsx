...
import { createHtmlOTPVerifyEmail } from '~/utils/formatters'
import cryptoRandomString from 'crypto-random-string'
import { createNewUserAPI, sendMailAPI } from '~/apis'
...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    const resCreate = await createNewUserAPI(createData)
    setIsProcessing(false)
    if (resCreate?.message) {
      toast[resCreate.type](resCreate.message, {
        position: 'top-center',
        autoClose: 2500,
    ...
    const htmlRegister = createHtmlOTPVerifyEmail(data)
    const to = emailInput
    await sendMailAPI({
      from: '"Trello"<khoindt10a4@gmail.com>',
      to: to,
      subject: `${data.OTP} là mã xác minh của bạn`,
      html: htmlRegister
    })

    const userInfo = {
      _id: resCreate.insertedId,
      email: emailInput
    }
