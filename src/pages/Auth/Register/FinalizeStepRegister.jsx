...
import { finalizeStepRegisterAPI } from '~/apis'
import { validatePassword } from '~/utils/algorithms'
import { paramsDecodeUrlBase64 } from '~/utils/formatters'

function FinalizeStepRegister() {
  ...
  const userInfo = paramsDecodeUrlBase64()

  const handleOnchangeInputFullName = (value) => {
  ...
  const handleOnchangeInputPassword = (value) => {
  ...
  const handleSubmitForm = async () => {
    const errors = {}
    if (!inputFullName) errors.fullName = 'Please enter your full name'
    if (!inputPassword) errors.password = 'Please enter your password'
    if (!inputConfirmPassword) errors.confirmPassword = 'Please enter your confirm password'
    if (inputPassword && inputConfirmPassword) {
      const errorVaLidatePassword = validatePassword(inputPassword)
      if (!isEmpty(errorVaLidatePassword)) {
        const errorPassword = 'Password must:\n' + Object.values(errorVaLidatePassword).join('\n')
        errors.password = errorPassword
      }
      const errorVaLidateConfirmPassword = validatePassword(inputConfirmPassword)
      if (!isEmpty(errorVaLidateConfirmPassword)) {
        const errorConfirmPassword = 'Confirm password must:\n' + Object.values(errorVaLidateConfirmPassword).join('\n')
        errors.confirmPassword = errorConfirmPassword
      }
      if (!errorVaLidatePassword && !errorVaLidateConfirmPassword) {
        if (inputConfirmPassword !== inputPassword) errors.confirmPassword = 'Password confirmation mismatch'
      }
    }
    setErrorForm(errors)
    if (isEmpty(errors)) {
      const resUpdate = await finalizeStepRegisterAPI({
        displayName: inputFullName,
        password: inputPassword,
        confirmPassword: inputConfirmPassword,
        _id: userInfo._id
      })
      if (resUpdate?.message) {
        toast[resUpdate.type](resUpdate.message, {
          position: 'top-center',
          autoClose: 2500,
          onClose: () => {
            if (resUpdate.type === 'success') {
          ...
          <Box
            ...
            <Box
              ...
              <Box>
                ...
                <TextField
                  value={inputFullName}
                  onChange={(e) => handleOnchangeInputFullName(e.target.value)}
                  autoComplete='off'
                  fullWidth
              ...
              <Box>
                ...
                <TextField
                  value={inputPassword}
                  onChange={(e) => handleOnchangeInputPassword(e.target.value)}
                  autoComplete='off'
                  fullWidth
              ...
              <Typography
                ...
                <Link
                  ...
                  the Atlassian Cloud Terms of Service
                </Link>
...
export default FinalizeStepRegister
